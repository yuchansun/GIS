...
let roadGraph = null;            // 真實道路網圖：{ nodes:[[lat,lng]], adj:[[{to,dist}]] }
let roadNodeRiverDist = null;    // 每個道路節點到最近河岸的距離（快取）
...
function runEvacuation(origin, originName) {
  ...
  // 用道路網 A* 算出「沿真實道路、避開河岸/淹水範圍」的安全路徑
  const destination = { lat: nearestShelter.lat, lng: nearestShelter.lng };
  const route = computeRoadRoute(origin, destination);
  ...
  // 記住目前避難資訊（起點、避難所、安全路徑），供導航與降雨重算使用
  lastEvacuation = { origin, name: originName, shelter: nearestShelter, path: route.path };
}
...
function compassZh(bearing) {
  const dirs = ['北', '東北', '東', '東南', '南', '西南', '西', '西北'];
  return dirs[Math.round(bearing / 45) % 8];
}

// 由安全路徑（道路節點序列）直接產生逐步轉彎指示（不經 Google，確保完全沿安全路線）
function buildNavSteps(path) {
  const pts = path.filter((p, i) => i === 0 || approxMeters(p.lat, p.lng, path[i - 1].lat, path[i - 1].lng) > 2);
  if (pts.length < 2) return [];
  const steps = [];
  let prevB = segBearing(pts[0], pts[1]);
  let runDist = approxMeters(pts[0].lat, pts[0].lng, pts[1].lat, pts[1].lng);
  let label = `出發，朝${compassZh(prevB)}方向前進`;
  for (let i = 1; i < pts.length - 1; i++) {
    const b = segBearing(pts[i], pts[i + 1]);
    const segLen = approxMeters(pts[i].lat, pts[i].lng, pts[i + 1].lat, pts[i + 1].lng);
    const delta = ((b - prevB + 540) % 360) - 180; // -180~180，正=右、負=左
    if (Math.abs(delta) >= 30) {
      steps.push({ text: label, dist: runDist });
      label = Math.abs(delta) > 150 ? '迴轉' : (delta > 0 ? '向右轉' : '向左轉');
      runDist = segLen;
    } else {
      runDist += segLen;
    }
  ...
  steps.push({ text: label, dist: runDist });
  steps.push({ text: '抵達避難所', dist: 0 });
  return steps;
}

// 🧭 頁面內逐步導航：完全沿著安全路線（綠線）產生轉彎指示
function startInPageNavigation() {
  const ev = lastEvacuation;
  if (!ev || !ev.path || !ev.shelter) {
    alert('請先點選受災點，產生避難路線後再開始導航。');
    return;
  }
  const panel = document.getElementById('nav-steps');
  if (!panel) return;
  const steps = buildNavSteps(ev.path);
  if (!steps.length) { alert('路徑太短，無法產生導航指示。'); return; }

  panel.style.display = 'block';
  panel.innerHTML =
    `<div style="font-weight:bold; color:#0d6efd; margin-bottom:6px;">🧭 沿安全路線步行指示（避開河岸與淹水範圍）</div>`
    + steps.map((s, i) => {
      const dist = s.dist > 0 ? `<span style="color:#888;">（約 ${Math.round(s.dist)} 公尺）</span>` : '';
      return `<div style="padding:6px 4px; border-bottom:1px solid #eee; font-size:13px;">${i + 1}. ${s.text} ${dist}</div>`;
    }).join('');
}

// 🚶 開啟外部 Google Maps 進行真實步行導航
function openExternalNavigation() {
  ...
  const url = `https://www.google.com/maps/dir/?api=1`
    + `&origin=${ev.origin.lat},${ev.origin.lng}`
    + `&destination=${ev.shelter.lat},${ev.shelter.lng}`
    + `&travelmode=walking`;
  window.open(url, '_blank');
}

// 🔀 左側面板收合 / 展開（含 RWD：手機預設收合，地圖自動補滿並重置中心）
function setupDashboardToggle() {
  const handle = document.getElementById('dash-handle');
  if (!handle) return;
  ...
  const apply = () => {
    const collapsed = document.body.classList.contains('dash-collapsed');
    handle.textContent = collapsed ? '›' : '‹';
    handle.setAttribute('aria-label', collapsed ? '展開面板' : '收合面板');
  };
  ...
  // 手機 / 平板預設收合，讓地圖先全屏顯示
  if (window.innerWidth <= 768) document.body.classList.add('dash-collapsed');
  apply();
}

// ↔ 拖曳面板右緣調整寬度（記憶到 localStorage；手機不啟用）
function setupPanelResize() {
  const resizer = document.getElementById('dash-resizer');
  if (!resizer) return;
  const root = document.documentElement;
  const MIN_W = 260;
  const maxW = () => Math.min(640, Math.round(window.innerWidth * 0.7));
  const setWidth = (w) => root.style.setProperty('--dash-w', Math.min(Math.max(w, MIN_W), maxW()) + 'px');

  // 還原上次設定（僅電腦版）
  const saved = parseInt(localStorage.getItem('dashWidth'), 10);
  if (saved && window.innerWidth > 768) setWidth(saved);
  ...
  const stop = () => {
    ...
    const px = parseInt(getComputedStyle(root).getPropertyValue('--dash-w'), 10);
    if (px) localStorage.setItem('dashWidth', px);
...
function goToSearchLocation(location, name) {
  if (!location || !map) return;
  map.panTo(location);
  map.setZoom(15);
  showUserLocation(location);
  triggerEvacuationGuidance(location, name || '搜尋地點');
  setSearchStatus(`✓ 已定位：${name || '搜尋地點'}，避難路線規劃中…`, 'ok');
}

async function geocodeQuery(query) {
  const q = (query || '').trim();
  if (!q) {
    setSearchStatus('請先輸入地點名稱或地址。', 'err');
    return;
  }
  setSearchStatus('搜尋中…');
  const { Geocoder } = await google.maps.importLibrary('geocoding');
  const geocoder = new Geocoder();
  geocoder.geocode({ address: q, componentRestrictions: { country: 'TW' } }, (results, status) => {
    if (status !== 'OK' || !results?.length) {
      setSearchStatus('找不到該地點，請換關鍵字或從下拉選單選擇。', 'err');
      return;
    }
    const best = results[0];
    goToSearchLocation(best.geometry.location, best.formatted_address || q);
  });
}
...
async function setupPlaceSearch() {
  ...
  document.getElementById('btn-place-search')?.addEventListener('click', () => {
    const place = autocomplete.getPlace();
    if (place?.geometry?.location) {
      goToSearchLocation(place.geometry.location, place.name || place.formatted_address || input.value);
    } else {
      geocodeQuery(input.value);
    }
  });
  ...
  input.addEventListener('keydown', (e) => {
    if (e.key === 'Enter') {
      e.preventDefault();
      document.getElementById('btn-place-search')?.click();
    }
  });
}
...
// =========================================================================
// 🛣️ 真實道路網 A*：路徑只走 OSM 道路節點，並依河岸/淹水/雨量加權避災
// =========================================================================

// 載入預先抓好的新莊步行道路網，建成鄰接表（含每條邊的實際長度）
async function loadRoadNetwork() {
  const data = await fetchJson('/road_network.json');
  if (!data || !Array.isArray(data.nodes) || !Array.isArray(data.edges)) {
    console.warn('道路網載入失敗，將回退到格網式 A*。');
    return;
  }
  ...
  for (const [a, b] of data.edges) {
    ...
    const dist = approxMeters(nodes[a][0], nodes[a][1], nodes[b][0], nodes[b][1]);
    adj[a].push({ to: b, dist });
    adj[b].push({ to: a, dist });
  }
  roadGraph = { nodes, adj };
  console.log(`道路網就緒：${nodes.length} 節點 / ${data.edges.length} 邊`);
}

// 預算每個道路節點到最近河岸的距離（河流為靜態資料，算一次即快取）
function ensureRoadNodeRiverDist() {
  if (roadNodeRiverDist) return roadNodeRiverDist;
  const nodes = roadGraph.nodes;
  const out = new Float32Array(nodes.length);
  const danger = buildDangerPoints();
  if (!danger.length) { out.fill(Infinity); roadNodeRiverDist = out; return out; }
  for (let i = 0; i < nodes.length; i++) {
    const lat = nodes[i][0], lng = nodes[i][1];
    let minD = Infinity;
    for (let j = 0; j < danger.length; j++) {
      const d = approxMeters(lat, lng, danger[j].lat, danger[j].lng);
      if (d < minD) { minD = d; if (minD <= 50) break; }
    }
    out[i] = minD;
  }
  roadNodeRiverDist = out;
  return out;
}
...
function nearestRoadNode(p) {
  ...
  for (let i = 0; i < nodes.length; i++) {
    const d = approxMeters(p.lat, p.lng, nodes[i][0], nodes[i][1]);
    if (d < bestD) { bestD = d; best = i; }
  }
  return best;
}

// 道路網 A*：回傳 { path:[{lat,lng}...], blocked }；無圖或失敗時回退格網式 A*
function computeRoadRoute(origin, dest) {
  if (!roadGraph) return computeSafeRoute(origin, dest);
  const { nodes, adj } = roadGraph;
  const n = nodes.length;
  const startNode = nearestRoadNode(origin);
  const goalNode = nearestRoadNode(dest);
  if (startNode < 0 || goalNode < 0) return computeSafeRoute(origin, dest);

  const riverDist = ensureRoadNodeRiverDist();
  const rainFactor = 1 + Math.min(currentRainfall, 40) / 20; // 1~3 倍，雨越大越避災
  const hasFlood = floodPolygons.length > 0;

  // 每個節點的「危險加權單位」：近河岸 / 落在淹水潛勢範圍內 → 成本大增
  const penCache = new Float32Array(n).fill(-1);
  const floodFlag = new Int8Array(n).fill(-1);
  const penaltyOf = (i) => {
    let p = penCache[i];
    if (p >= 0) return p;
    let units = 0;
    const rd = riverDist[i];
    if (rd <= 50) units += 4;        // 緊鄰河道
    else if (rd <= 120) units += 2;  // 近河岸
    if (hasFlood) {
      let f = floodFlag[i];
      if (f === -1) { f = pointInFlood(nodes[i][1], nodes[i][0]) ? 1 : 0; floodFlag[i] = f; }
      if (f) units += 8;             // 落在官方淹水潛勢範圍內 → 強烈避開
    }
    p = units * rainFactor;
    penCache[i] = p;
    return p;
  ...
  const gx = nodes[goalNode][0], gy = nodes[goalNode][1];
  const h = (i) => approxMeters(nodes[i][0], nodes[i][1], gx, gy);
  ...
  open.push(startNode, h(startNode));
  ...
  while (!open.isEmpty()) {
    ...
    const penCur = penaltyOf(cur);
    const list = adj[cur];
    for (let k = 0; k < list.length; k++) {
      const e = list[k];
      if (closed[e.to]) continue;
      // 邊權重 = 實際長度 ×（1 + 兩端危險加權平均）
      const weight = e.dist * (1 + (penCur + penaltyOf(e.to)) / 2);
      const tentative = curG + weight;
  ...
  if (!found) return computeSafeRoute(origin, dest); // 道路不連通時退回格網結果

  const path = [];
  let c = goalNode;
  while (c !== -1) {
    path.unshift({ lat: nodes[c][0], lng: nodes[c][1] });
    if (c === startNode) break;
    c = cameFrom[c];
  }
  path.unshift(origin); // 精確起點接到最近道路節點
  path.push(dest);       // 最近道路節點接到避難所
  return { path, blocked: false };
}

// 主演算法（格網式 A*，作為道路網的後備）：回傳 { path: [{lat,lng}...], blocked }
...
function drawSafeRoute(path) {
  ...
  const navPanel = document.getElementById('nav-steps');
  if (navPanel) { navPanel.style.display = 'none'; navPanel.innerHTML = ''; }
...
async function initMap() {
  ...
  directionsRenderer = new DirectionsRenderer({
    ...
    polylineOptions: {
      strokeColor: "#1a73e8", // 藍色：沿安全路線的實際道路導航
      strokeWeight: 6,
      strokeOpacity: 0.9
    }
  });
  ...
  loadRoadNetwork(); // 背景載入道路網，供避災路徑規劃
  setupRainfall();
  ...
  document.getElementById('btn-nav-steps')?.addEventListener('click', startInPageNavigation);
  document.getElementById('btn-nav-external')?.addEventListener('click', openExternalNavigation);
